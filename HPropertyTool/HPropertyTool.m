...
-(void)saveDict:(NSDictionary<NSString *, NSDictionary *> *)dict ToPath:(NSString *)path{
    ...
    [dict enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, NSDictionary * _Nonnull obj, BOOL * _Nonnull stop) {
        ...
        BOOL isSucc = YES;
        [pf.hString writeToFile:[NSString stringWithFormat:@"%@/%@.h",path,key.uppercaseFirstChar] atomically:YES encoding:NSUTF8StringEncoding error:&error];
        if (error) {
            NSLog(@"%@的.h文件写入失败:%@ %@",key,error,error.userInfo);
            isSucc = NO;
        }
        
        [pf.mString writeToFile:[NSString stringWithFormat:@"%@/%@.m",path,key.uppercaseFirstChar] atomically:YES encoding:NSUTF8StringEncoding error:&error];
        if (error) {
